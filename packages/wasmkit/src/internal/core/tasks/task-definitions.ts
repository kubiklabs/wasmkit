...
export class SimpleTaskDefinition implements TaskDefinition {
  ...
  constructor (
    ...
    this.action = () => {
      throw new WasmkitError(ERRORS.TASK_DEFINITIONS.ACTION_NOT_SET, {
        taskName: name
      });
    };
  }
  ...
  private _validateNotAfterVariadicParam (name: string): void {
    if (this._hasVariadicParam) {
      throw new WasmkitError(ERRORS.TASK_DEFINITIONS.PARAM_AFTER_VARIADIC, {
        paramName: name,
        taskName: this.name
      });
    }
  ...
  private _validateNameNotUsed (name: string): void {
    if (this._hasParamDefined(name)) {
      throw new WasmkitError(ERRORS.TASK_DEFINITIONS.PARAM_ALREADY_DEFINED, {
        paramName: name,
        taskName: this.name
      });
    }
...
export class OverriddenTaskDefinition implements TaskDefinition {
  ...
  private _throwNoParamsOverrideError (errorDescriptor: ErrorDescriptor): never {
    throw new WasmkitError(errorDescriptor, {
      taskName: this.name
    });
  }
}
