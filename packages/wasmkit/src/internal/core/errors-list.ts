...
export const ERRORS = {
  GENERAL: {
    NOT_INSIDE_PROJECT: {
      number: 1,
      message: 'You are not inside a WasmKit project.',
      title: 'You are not inside a WasmKit project',
      description: `You are trying to run WasmKit outside of a WasmKit project.
...
You can learn how to use WasmKit by reading the [Getting Started guide](../getting-started).`,
    ...
    INVALID_NODE_VERSION: {
      number: 2,
      message:
        "WasmKit doesn't support your Node.js version. It should be %requirement%.",
      title: 'Unsupported Node.js',
      description: `WasmKit doesn't support your Node.js version.
...
Please upgrade your version of Node.js and try again.`,
    ...
    UNSUPPORTED_OPERATION: {
      number: 3,
      message: '%operation% is not supported in WasmKit.',
...
Please [report it](https://github.com/arufa-research/wasmkit/issues/new) to help us improve WasmKit.`,
    ...
    CONTEXT_PRE_NOT_DEFINED: {
      number: 6,
      message:
        'WasmKit Runtime Environment is not defined in the WasmkitContext.',
...
Please [report it](https://github.com/arufa-research/wasmkit/issues/new) to help us improve WasmKit.`,
    ...
    CONTEXT_PRE_ALREADY_DEFINED: {
      number: 7,
      message:
        'WasmKit Runtime Environment is already defined in the WasmkitContext',
...
To learn more about WasmKit's configuration, please go to https://wasmkit.dev/config/`,
      title: 'Invalid WasmKit config',
      description: `You have one or more errors in your config file.

Check the error message for details, or go to [documentation](https://wasmkit.dev/config/) to learn more.`,
      shouldBeReported: false
    },
    LIB_IMPORTED_FROM_THE_CONFIG: {
      number: 9,
      message: `Error while loading WasmKit's configuration.
You probably imported WasmKit instead of WasmKit/config`,
      title: 'Failed to load config file',
      description: `There was an error while loading your config file.

The most common source of errors is trying to import \`WasmKit\` instead of \`WasmKit/config\`.
...
Please [report it](https://github.com/arufa-research/wasmkit/issues/new) to help us improve WasmKit.`,
    ...
    NON_LOCAL_INSTALLATION: {
      number: 12,
      message:
        'Trying to use a non-local installation of WasmKit, which is not supported.\nPlease install WasmKit locally using npm or Yarn, and try again.',
      title: 'WasmKit is not installed or installed globally',
      description: `You tried to run WasmKit from a global installation or not installing it at all. This is not supported.
...
Please install WasmKit locally using npm or Yarn, and try again.`,
    ...
    TS_NODE_NOT_INSTALLED: {
      number: 13,
      message: `Your WasmKit project uses typescript, but ts-node is not installed.

Please run: npm install --save-dev ts-node`,
      title: 'ts-node not installed',
      description: `You are running a WasmKit project that uses typescript, but you haven't installed ts-node.
...
Please run this and try again: npm install --save-dev ts-node`,
    ...
    TYPESCRIPT_NOT_INSTALLED: {
      number: 14,
      message: `Your WasmKit project uses typescript, but it's not installed.

Please run: npm install --save-dev typescript`,
      title: 'typescript not installed',
      description: `You are running a WasmKit project that uses typescript, but it's not installed.
...
Please check that the configured keypair are correct.`,
  ...
  NETWORK: {
    CONFIG_NOT_FOUND: {
      ...
      description: `You are trying to run WasmKit with a non-existent network.

Read the [documentation](https://WasmKit.org/config/#networks-configuration) to learn how to define custom networks.`,
      shouldBeReported: false
    },
    INVALID_GLOBAL_CHAIN_ID: {
      number: 101,
      message:
        'WasmKit was set to use chain id %configChainId%, but connected to a chain with id %connectionChainId%.',
      title: 'Connected to the wrong network',
      description: `Your config specifies a chain id for the network you are trying to used, but WasmKit detected anotherone.
...
account not managed by your Ethereum node nor WasmKit.
...
Read the [documentation](https://WasmKit.org/config/#hd-wallet-config) to learn how to define HD accounts correctly.`,
      shouldBeReported: false
    },
...
Please double check your task definitions.`,
    ...
    PARAM_CLASHES_WITH_WASMKIT_PARAM: {
      number: 202,
      message:
        'Could not set param %paramName% for task %taskName% because its name is used as a param for WasmKit.',
      title: 'WasmKit and task param names clash',
      description: `Could not add a param to your task because its name is used as a param for WasmKit.
...
What makes these types special is that they can be represented as strings, so you can write them down in the terminal.`,
  ...
  ARGUMENTS: {
    INVALID_ENV_VAR_VALUE: {
      ...
      description: `You are setting one of WasmKit arguments using an environment variable, but it has an incorrect value.
...
Please double check your environment variables.`,
    ...
    INVALID_VALUE_FOR_TYPE: {
      ...
      title: 'Invalid argument type',
      description: `One of your WasmKit or task's arguments has an invalid type.
...
Please double check the name of the task you are trying to run.`,
    ...
    UNRECOGNIZED_COMMAND_LINE_ARG: {
      ...
      description: `WasmKit couldn't recognize one of your command line arguments.

This may be because you are writing it before the task name. It should come after it.

Please double check how you invoked WasmKit.`,
      shouldBeReported: false
    },
    UNRECOGNIZED_PARAM_NAME: {
      ...
      description: `WasmKit couldn't recognize one of your tasks' parameters.

Please double check how you invoked WasmKit or run your task.`,
      shouldBeReported: false
    },
...
Please double check how you invoked WasmKit or run your task.`,
    ...
    PARAM_NAME_INVALID_CASING: {
      ...
      description: `You tried to run WasmKit with a parameter with invalid casing. They must be lowercase.
...
You must always use slashes (/) in Solidity imports.`,
    ...
    INVALID_IMPORT_PROTOCOL: {
      number: 406,
      message:
        "Invalid import %imported% from %from%. WasmKit doesn't support imports via %protocol%.",
...
You can only import files thar are available locally or installed through npm.`,
    ...
    INVALID_IMPORT_ABSOLUTE_PATH: {
      number: 407,
      message:
        "Invalid import %imported% from %from%. WasmKit doesn't support imports with absolute paths.",
...
This is not supported by WasmKit.`,
      shouldBeReported: false
    },
...
WasmKit's compiler is case sensitive to ensure projects are portable across different operating systems.`,
      shouldBeReported: false
    },
...
Please check WasmKit's output for more details.`,
      shouldBeReported: false
    },
    RUN_FILE_NOT_FOUND: {
      number: 601,
      ...
      description: `Tried to use \`WasmKit run\` to execut a non-existing script.
...
Please check WasmKit's output for more details.`,
    ...
    FLATTEN_CYCLE: {
      number: 603,
      message: "WasmKit flatten doesn't support cyclic dependencies.",
      title: 'Flatten detected cyclic dependencies',
      description: `WasmKit flatten doesn't support cyclic dependencies.
...
We recommend not using this kind of dependencies.`,
    ...
    JSONRPC_UNSUPPORTED_NETWORK: {
      number: 605,
      message:
        'Unsupported network for JSON-RPC server. Only WasmKit is currently supported.',
      title: 'Unsupported network for JSON-RPC server.',
      description: `JSON-RPC server can only be started when running the WasmKit Network.
...
%reasons%`,
    ...
    NODE_FORK_BLOCK_NUMBER_WITHOUT_URL: {
      ...
      description: `You passed a block number to fork from, but not URL. WasmKit cannot fork
...
or go to [solang](https://solang.readthedocs.io/en/latest/installing.html) to learn more.`,
    ...
    INK_NOT_FOUND_ARTIFACT: {
      number: 610,
      message: `Ink compiles successfully, but WasmKit cannot find the compiled artifact.`,
      title: 'Ink not found arifacts',
      description: `If you think this is a bug in WasmKit, please report it here: https://github.com/patractlabs/WasmKit/issues/new`,
      shouldBeReported: false
    },
    INK_DUPLICATE_NAME: {
      number: 611,
  ...
  Please double check your command parameters`,
    ...
    RUN_FILES_NOT_FOUND: {
      ...
      description: `Tried to use \`wasmkit run\` to execute a non-existing script(s).
...
`,
      ...
      description: `There are multiple artifacts that match the given contract name, and WasmKit doesn't know which one to use.
...
WasmKit's artifact resolution is case sensitive to ensure projects are portable across different operating systems.`,
      shouldBeReported: true
    },
...
export const ERROR_RANGES: {
  ...
  INTERNAL: { min: 900, max: 999, title: 'Internal WasmKit errors' },
